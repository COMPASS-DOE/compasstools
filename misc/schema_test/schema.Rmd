...
Let's say we have one site (Crane Creek), and two plot (transition and upland).
Each plot has a datalogger monitoring two trees, one with a deep sensor, as well
as soil moisture.
There's also a met datalogger for the entire site that measures PAR.
...
Note that I'm using "comma notation" here in the plot table, so expand it before continuing:

```{r}
plot_data_ex <- expand_df(plot_data)
datatable(plot_data_ex) %>% 
    formatStyle("design_link", color = "red") %>% 
    formatStyle("Logger", color = "green")
```
...
```{r}
dat1 %>% 
    left_join(plot_table, by = c("Logger", "design_link")) ->
    dat2
datatable(dat2)
```
